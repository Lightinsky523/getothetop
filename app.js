...
// 全局 SMTP transporter：端口 465 用 secure，587 用 STARTTLS；避免每次发信新建连接导致 Unexpected socket close
...
if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
  try {
    ...
    mailTransporter.verify((err, success) => {
      if (err) {
        console.error('SMTP 初始化失败:', err);
        mailTransporter = null;
      } else {
        console.log('✅ SMTP 邮件服务初始化成功');
      }
    });
  } catch (e) {
    console.error('SMTP 创建异常:', e);
    mailTransporter = null;
  }
}
