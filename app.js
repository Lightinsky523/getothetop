...
// `express.json({ limit })` 底层使用 raw-body，`limit:false` 在部分版本/封装下并不一定真的等于“无上限”。
// 为避免 `entity.too.large`，这里给一个很大的上限。
// - 默认：250mb（足够覆盖 base64 大图场景）
// - 若设置环境变量 `BODY_LIMIT=unlimited`：使用 1024mb 代替（仍需注意上游网关可能有更小限制）
...
// ========== 学生分享调试接口（生产环境建议关闭）==========
// 直接查询数据库，不应用复杂的过滤条件
app.get('/api/debug/shares-all', async (req, res) => {
  try {
    // 检查 MySQL 连接
    if (!mysqlPool) {
